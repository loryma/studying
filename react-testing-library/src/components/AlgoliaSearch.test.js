...
describe("App", () => {
  it("fetches news from API", async () => {
    ...
    const { getByRole, findAllByRole } = render (<AlgoliaSearch />);
  ...
  it("fetches news from API and reject", async () => {
    ...
    const { getByRole, findByText } = render (<AlgoliaSearch />);
    userEvent.click(getByRole("button"));
  ...
  it("fetches news from API in alternative way", async () => {
    const promise = Promise.resolve({data: {hits}})
    axios.get.mockImplementationOnce(() => promise);
    const { getByRole, getAllByRole } = render (<AlgoliaSearch />);
    userEvent.click(getByRole("button"));
    await act(() => promise);
